...
// --- Statikus globális változók a bemeneti kezeléshez ---
...
/* 
Ez a függvény frissíti a `KeyState` struktúrában a billentyűk állapotát (lenyomva vagy nem).
Minden érdekesebb billentyűre (pl. w, a, s, d, enter stb.) külön flag van.
*/
...
// Ez a függvény visszaadja, hogy egy adott billentyű épp le van-e nyomva a `KeyState` alapján.
...
//Inicializálja a bemeneti kezelést: beállítja az alapértelmezett billentyűállapotot (üres) és elmenti a játék/működési állapotokra mutató pointereket.
...
/*
Ez a függvény egy SDL eseményt dolgoz fel globálisan.
A `key_state` alapján módosítja a játékállapotot, menüt, fényerőt, kamerát stb.
*/
