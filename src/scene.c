...
/*
Fényforrások inicializálása a jelenethez.
Összesen 4 különböző fényforrás kerül létrehozásra, különböző pozíciókkal és tulajdonságokkal.
 */
    void init_lights(Scene* scene, int brightness) {
        scene->n_fenyek = 4;
    ...
    scene->fenyek[1].ambient[0] = (GLfloat)brightness / 255.0f;
    scene->fenyek[1].diffuse[0] = 0.5f;
    scene->fenyek[1].diffuse[1] = 0.5f;
    scene->fenyek[1].specular[0] = 0.7f;
...
/*
A jelenet összes modelljének és textúrájának betöltése és inicializálása.
Itt történik meg minden modell betöltése, transzformációk beállítása, textúrák hozzárendelése.
*/
    void init_scene(Scene* scene){
    // AJTO 4db
        //init_model(&(scene->ajtofelfa1));
...
//Textúrák betöltése
    ...
    void update_scene(Scene* scene, const App* app, double elapsed_time) {
        if (!scene || !app) return;

        // Fényforrás mozgatása (a light.c-ben van definiálva a move_light)
        // Az app->move_light_x/y/z flags jelzi a mozgásirányt
        if (app->move_light_x) {
            scene->fenyek[0].position[0] += app->light_move_speed * elapsed_time;
            glLightfv(scene->fenyek[0].id, GL_POSITION, scene->fenyek[0].position);
        }
        if (app->move_light_y) {
            scene->fenyek[0].position[1] += app->light_move_speed * elapsed_time;
            glLightfv(scene->fenyek[0].id, GL_POSITION, scene->fenyek[0].position);
        }
        if (app->move_light_z) {
            scene->fenyek[0].position[2] += app->light_move_speed * elapsed_time;
            glLightfv(scene->fenyek[0].id, GL_POSITION, scene->fenyek[0].position);
        } 
}
